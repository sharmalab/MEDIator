...
A prototype web application is built with the data replication and synchronization tool and TCIA REST API. Apache Tomcat (Embedded) is integrated into the program such that it will get the user inputs from the HTML pages and present the output in pages formatted by Apache Velocity Templates. Respective servlets are created inside the servlets package to receive the inputs from HTML pages to the backend Java code.

The Replication Tool is multi-tenanted, and it is aware of the multiple tenants or users using the system.

createReplicaSet(),
For the below actions, "userID" should be input along with the actions.
1. createReplicaSet.
2. deleteReplicaSet (both replicaSetID and userID)
3. getAllReplicaSets of the user.
4. duplicateReplicaSet (both replicaSetID and userID. Here the user gets the replicaSet ID of another user's replica set. He will duplicate it into another replica set as his own, where now the duplicate will assume another replica set ID).

updateReplicaSet does not require the userID. But it requires replicaSetID.

ReplicaSetID is a large minus number UUID randomly generated. It can be safely assumed to be harder to guess. Hence, it is assumed to provide enough security for this prototype.

User ID is a random string that is input by the user. Probably this can be some 'secret' or pass from the user. userIDs serve as the keys of the userReplicasMap. Apart from that, there is no data structure to hold the list of users.
